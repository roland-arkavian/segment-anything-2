...
def load_video_frames(
    ...
    for n in tqdm(range(num_frames), desc="frame loading (JPEG)"):
        img_pil = Image.fromarray(image_list[n])
        img_np = np.array(img_pil.convert("RGB").resize((image_size, image_size)))
        if img_np.dtype == np.uint8:  # np.uint8 is expected for JPEG images
            img_np = img_np / 255.0
        else:
        ...
        images[n], video_height, video_width = img, video_height, video_width
